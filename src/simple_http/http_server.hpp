...
namespace http_server {
    ...
    void HttpServer::HandleHttpData(const EventData& raw_request, EventData *raw_response) {
        ...
        try {
            http_request = http::string_to_request(request_string);
            http_response = HandleHttpRequest(http_request);
        } catch (const std::invalid_argument& e) {
            http_response = http::HttpResponse(http::HttpStatusCode::BadRequest);
            http_response.SetContent(e.what());
        } catch (const std::logic_error& e) {
            http_response = http::HttpResponse(http::HttpStatusCode::HttpVersionNotSupported);
            http_response.SetContent(e.what());
        } catch (const std::exception& e) {
            http_response = http::HttpResponse(http::HttpStatusCode::InternalServerError);
            http_response.SetContent(e.what());
        }

        // Set response to write to client
        response_string = to_string(http_response, http_request.method() != http::HttpMethod::HEAD);
        strncpy(raw_response->buffer, response_string.c_str(), kMaxBufferSize);
        raw_response->length = response_string.length();
    }

    http::HttpResponse HttpServer::HandleHttpRequest(const http::HttpRequest& request) {
        auto it = request_handlers_.find(request.uri());
        if (it == request_handlers_.end()) {    // this uri is not registered
            return http::HttpResponse(http::HttpStatusCode::NotFound);
        }
        auto callback_it = it->second.find(request.method());
        if (callback_it == it->second.end()) {  // no handler for this method
            return http::HttpResponse(http::HttpStatusCode::BadGateway);
        }
